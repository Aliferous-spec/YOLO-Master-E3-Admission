"""Shared record-building core for the MoE / MoT / Latent routing adapters.

Every family snapshot reduces to the same ``e3-routing/v1`` record: validate
the required keys, convert tensors to plain values, reuse the upstream
``global_routing_metrics`` helper for entropy / gini, derive sum-to-one
shares and the dominant expert, clamp normalized entropy to ``[0, 1]``, and
copy only whitelisted keys into ``family_data``.  Family-specific extraction
(aux-loss finiteness, ``family_data`` value policy) stays in each adapter
subclass behind the small hooks below.
"""
...
class RoutingAdapterBase:
    """One ``to_record`` template; subclasses supply family specifics."""

    FAMILY = ""
    ROUTING_PARADIGM = ""
    REQUIRED_SNAPSHOT_KEYS = ("num_experts", "top_k", "expert_usage")
    FAMILY_DATA_WHITELIST: tuple[str, ...] = ()
    ...
    def _to_plain(value: Any) -> Any:
        ...
        if isinstance(value, dict):
            return {str(key): RoutingAdapterBase._to_plain(item) for key, item in value.items()}
        if isinstance(value, (list, tuple)):
            return [RoutingAdapterBase._to_plain(item) for item in value]
    ...
    def _snapshot_label(self) -> str:
        """Family label used in error text, e.g. ``MoEAdapter`` -> ``MoE``."""
        name = type(self).__name__
        return name.removesuffix("Adapter")

    def _snapshot_finite(self, plain: Mapping[str, Any]) -> bool | None:
        """Return the family aux-loss finite flag, or None when absent."""
        return None

    def _family_data_value(self, key: str, value: Any) -> Any:
        """Prepare one whitelisted ``family_data`` value for the record."""
        return value
    ...
    def to_record(
        ...
        *,
        ...
        """Build one validated RoutingRecord from an upstream family snapshot."""
        ...
        if missing:
            raise ValueError("missing required " + label + " snapshot field(s): " + ", ".join(missing))
        ...
        if usage_raw is None:
            raise ValueError(label + " snapshot expert_usage is None; no usage vector to record")
        ...
        normalized_entropy = min(max(entropy_nats / denom, 0.0), 1.0) if denom > 0.0 else 0.0
        ...
        for key in self.FAMILY_DATA_WHITELIST:
            ...
            if value is not None:
                family_data[key] = self._family_data_value(key, value)
